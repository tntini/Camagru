<?php
  ...
  /*if (empty($_POST[login]) || empty($_POST[passwd]) || empty($_POST[passwd2]) || empty($_POST[email])) {
      header("Location: ../index.php?err=Please fill in all the blanks.\n");
      exit();
  }*/
  if ($_POST[passwd] != $_POST[passwd2]) {
      header("Location: ../edit.php?err=Passwords do not match.\n");
      exit();
  }
  ...
  if (!$sth->fetchColumn()) {
      header("Location: ../edit.php?err=Account does not exist.\n");
      exit();
  }
  ...
  if ($sth->fetchColumn()) {
      try {
          $sth = $dbh->prepare("UPDATE users SET passwd = :passwd WHERE login = :login AND mail = :email AND forgot = :hash");
